...
void LCD_Init() {
	...
	MCAL_GPIO_WritePin(&DataDir_LCD_CTRL_PORT, EN_SWITCH, GPIO_PIN_SET);
	MCAL_GPIO_WritePin(&DataDir_LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_SET);
	MCAL_GPIO_WritePin(&DataDir_LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_SET);
	...
	MCAL_GPIO_WritePort(&DataDir_LCD_PORT, 0xFF);
	_delay_ms(15);
...
void LCD_Write_Command(unsigned char command) {
	...
	MCAL_GPIO_WritePort(&LCD_PORT, ((LCD_PORT & 0x0F)  |  (command & 0xF0)));
	//LCD_CTRL_PORT &= ~((0b1 << RS_SWITCH)  |  (0b1 << RW_SWITCH));
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_CLEAR);
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_CLEAR);
	_delay_ms(1);
	...
	MCAL_GPIO_WritePort(&LCD_PORT, ((LCD_PORT & 0x0F)  |  (command << DATA_shift)));
	//LCD_CTRL_PORT &= ~((0b1 << RS_SWITCH)  |  (0b1 << RW_SWITCH));
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_CLEAR);
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_CLEAR);
	_delay_ms(1);
...
void LCD_Write_Char(unsigned char character) {
	...
	MCAL_GPIO_WritePort(&LCD_PORT, ((LCD_PORT & 0x0F)  |  (character & 0xF0)));
	//LCD_CTRL_PORT |=  (0b1 << RS_SWITCH);		//turn RS ON for Data mode.
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_SET);
	//LCD_CTRL_PORT &= ~(0b1 << RW_SWITCH);		//turn RW OFF so you can write.
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_CLEAR);
	...
	MCAL_GPIO_WritePort(&LCD_PORT, ((LCD_PORT & 0x0F)  |  (character << DATA_shift)));
	//LCD_CTRL_PORT |=  (0b1 << RS_SWITCH);		//turn RS ON for Data mode.
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_SET);
	//LCD_CTRL_PORT &= ~(0b1 << RW_SWITCH);		//turn RW OFF so you can write.
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_CLEAR);
...
void LCD_Check_Is_LCD_Busy() {
	...
	MCAL_GPIO_WritePort(&DataDir_LCD_PORT, (DataDir_LCD_PORT & ~(0xFF << DATA_shift)));
	//LCD_CTRL_PORT |=  (1 << RW_SWITCH);
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RW_SWITCH, GPIO_PIN_SET);
	//LCD_CTRL_PORT &= ~(1 << RS_SWITCH);
	MCAL_GPIO_WritePin(&LCD_CTRL_PORT, RS_SWITCH, GPIO_PIN_CLEAR);
	
	if (MCAL_GPIO_ReadPin(&LCD_PIN, GPIO_PIN_7)) {
		LCD_Kick();
	}
